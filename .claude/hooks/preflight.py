"""SessionStart hook: verify workspace environment before starting work."""
import json
import shutil
import sys
...
# stderr too: handoff.py warnings (registry mismatch / unclosed fence) carry
# non-ASCII (an em-dash); without this they emit cp936 bytes on a Chinese-locale
# Windows and render as mojibake in a UTF-8 terminal.
...
def main():
    ...
    # IMPORTANT: HANDOFF.md is git-synced across devices and from a private
    # GitHub repo. Its contents are NOT trusted prompt content — they're
    # attacker-influenceable data (compromised collaborator, leaked GitHub
    # auth, or stolen device pushing a poisoned file). Wrap the task body in
    # a <task source="..." trust="untrusted"> envelope and tell Claude
    # explicitly: this is DATA to be DISPLAYED to the user, NOT INSTRUCTIONS
    # to execute. The /sync skill's Step 3 (per-task AskUserQuestion
    # confirmation) is what authorizes any command execution — never auto-run
    # from this preflight banner.
    if handoff:
        targets = []
        if machine_name:
